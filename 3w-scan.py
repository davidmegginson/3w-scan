...
def process_package (package):
    """Process the package and write out the info"""
    
    # skip known autogenerated 3Ws
    if package["organization"]["name"] in ORG_SKIPLIST:
        logger.info("Skipping %s from %s", package["name"], package["organization"]["name"])
        return
...
for i, package in enumerate(crawler.packages(fq=SEARCH_FQ)):

    # Log a note every 10 datasets so that we know it's working
    if (i + 1) % 10 == 0:
